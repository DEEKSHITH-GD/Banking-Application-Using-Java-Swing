...
{ 
	...
	{     
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); //closes the current window]
		...
		textField5 = new JTextField(20);    //set length for the text
		...
		textField6 = new JTextField(20);    //set length for the text
		...
		textField7 = new JTextField(10);    //set length for the text
		...
		textField8 = new JTextField(15);    //set length for the text
	...
	public void actionPerformed(ActionEvent ae)     //pass action listener as a parameter  
	{
		try {
			...
			if (br == true) {  //if true, navigate user to a new page  
				JOptionPane.showMessageDialog(button1, "RegistrationSuccessfull: "+BankingApplication.getCustomerName()+" with A/C no as "+BankingApplication.getAccountNo());
			}  
			else{  
				JOptionPane.showMessageDialog(button1, "Registration Failed");  
			}  
		}catch (Exception e) {
			JOptionPane.showMessageDialog(button1, "Registration Failed: "+e.toString());
		}  
	}  
}
